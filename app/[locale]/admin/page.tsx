    ...
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        ...
        <div className="flex items-center space-x-2 bg-slate-100 px-3 py-1.5 rounded-xl border border-slate-200 text-xs font-semibold text-slate-700">
          ...
          <span>Kayıtlı Balık: {fishes.length}</span>
        </div>
      </div>
      ...
      {/* Main Grid: Add Form & Existing List */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Left Side: Add Fish Form (Card) */}
        <div className="lg:col-span-7 bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-sm space-y-6">
          <div className="flex items-center space-x-2 border-b border-slate-100 pb-4">
            ...
            <h2 className="text-lg font-bold text-[#0F172A]">{t('addFish')}</h2>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 text-xs sm:text-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  name="name_tr"
                  required
                  placeholder="Örn: Aynalı Sazan"
                  value={formData.name_tr}
                  onChange={handleChange}
                  className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:ring-2 focus:ring-[#10B981] focus:border-transparent outline-none transition-all"
                />
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Örn: Cyprinus carpio"
                  value={formData.scientific_name || ''}
                  onChange={handleChange}
                  className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 italic focus:ring-2 focus:ring-[#10B981] focus:border-transparent outline-none transition-all"
                />
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={handleChange}
                  className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:ring-2 focus:ring-[#10B981] focus:border-transparent outline-none transition-all font-medium"
                >
            ...
            <div className="grid grid-cols-1 gap-4">
              <div>
                <label className="block font-semibold text-slate-700 mb-1">
                  {t('descriptionTr')} (Nehir/Göl/Deniz Avcılığı Detayları)
                </label>
                <textarea
                  name="description_tr"
                  rows={3}
                  placeholder="Balığın davranış yapısı, beslenme alışkanlıkları ve meralardaki bulunma durumları..."
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 px-4 bg-[#0F172A] hover:bg-slate-800 text-white font-bold rounded-xl transition-all shadow-md flex items-center justify-center space-x-2 disabled:opacity-50"
            >
              {loading ? (
                <>
        ...
        {/* Right Side: Registered Fishes List (Card) */}
        <div className="lg:col-span-5 bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-sm space-y-6 flex flex-col">
          <div className="flex items-center justify-between border-b border-slate-100 pb-4">
            <div className="flex items-center space-x-2">
              <List className="w-5 h-5 text-[#0F172A]" />
              <h2 className="text-lg font-bold text-[#0F172A]">{t('fishList')}</h2>
            </div>
          ...
          {fetching ? (
            ...
            <div className="space-y-3 overflow-y-auto max-h-[600px] pr-1">
              {fishes.map(fish => (
                <div
                  key={fish.id}
                  className="p-3.5 rounded-xl border border-slate-100 bg-slate-50 hover:bg-slate-100/80 transition-colors flex items-center justify-between space-x-3"
                >
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center space-x-2">
                      <h4 className="font-bold text-slate-900 text-sm truncate">{fish.name_tr}</h4>
                      <span className="text-xs text-slate-400 italic shrink-0">({fish.name_en})</span>
                    </div>
                    <p className="text-xs text-slate-500 truncate mt-0.5">
                      {fish.water_type || 'Tatlı Su'} • {fish.scientific_name || 'N/A'}
                    </p>
                  </div>
