...
async function applyUpdates() {
  console.log('Applying updates to Supabase directly...');
  let successCount = 0;
  ...
  for (const art of articles) {
    ...
    if (title_tr === 'Fly Fishing') title_tr = 'Fly Fishing (Sinek / Kamçı Balıkçılığı)';
    else if (title_tr === 'Albright Knot') title_tr = 'Albright Düğümü';
    else if (title_tr === 'Blood Knot') title_tr = 'Kan Düğümü (Blood Knot)';
    else if (title_tr === 'Improved Clinch Knot') title_tr = 'Gelişmiş Clinch Düğümü';
    else if (title_tr === 'Snell Knot') title_tr = 'Snell Düğümü (İğne Bağlama)';
    else if (title_tr === 'Uni Knot (Hang‑Knot)') title_tr = 'Uni Düğümü (Tekli ve Çiftli Uni)';
    else if (title_tr === 'Stainless Steel Leader') title_tr = 'Paslanmaz Çelik Tel Lider';
    else if (title_tr === 'Kevlar Leader') title_tr = 'Kevlar Lider Misina';
    ...
    } else if (title_tr === 'Fluorocarbon Kaplı Misina') title_tr = 'Fluorocarbon Kaplı Örgü İp Misina';
    else if (title_tr === 'Buzzbait (Vibrating Topwater)') title_tr = 'Buzzbait (Pervaneli Su Üstü Yemi)';
    ...
    } else if (title_tr === 'Swimbait (Soft Body)') title_tr = 'Swimbait (Yumuşak Gövdeli Balık Sahtesi)';
    else if (title_tr === 'Metal Jig (Heavy Metal)') title_tr = 'Ağır Metal Jig (Heavy Jig)';
    else if (title_tr === 'Jigging Rig (Heavy Jig)') title_tr = 'Jigging Takımı (Heavy Jig Rig)';
    else if (title_tr === 'Bobber Rig (Standard Float)') title_tr = 'Standart Şamandıralı Takım (Bobber Rig)';
    else if (title_tr === 'Lead‑Core (Tuna) Rig') title_tr = 'Kurşun Özlü Orkinos Takımı (Lead-Core Rig)';
    else if (title_tr === 'Carolina Rig') title_tr = 'Carolina Takımı (Carolina Rig)';
    else if (title_tr === 'Texas Rig') title_tr = 'Texas Takımı (Texas Rig)';
    else if (title_tr === 'Hybrid Drop‑Shot / Float Rig') title_tr = 'Hibrit Drop-Shot & Şamandıra Takımı';
    else if (title_tr === 'Drop Shot Rig') title_tr = 'Drop Shot Takımı';
    else if (title_tr === 'Pyramid & Split‑Shot Sinkers') title_tr = 'Piramit & Sıkıştırma Kurşunlar';
    else if (title_tr === 'Standard J‑Hook') title_tr = 'Standart J-İğne (J-Hook)';
    else if (title_tr === 'Rod Pod / Tripod') title_tr = 'Kamış Sehpası (Rod Pod / Tripod)';
    else if (title_tr === 'LED Light / Glow Stick') title_tr = 'Işıldak & LED Aydınlatma (Glow Stick)';
    else if (title_tr === 'Pliers / Scissors') title_tr = 'Balıkçı Pensi & Misina Makası';
    else if (title_tr === 'Fly Reel (Multiplik)') title_tr = 'Fly Çıkrık Makinesi';
    else if (title_tr === 'Bite Alarm (İsırma Alarmı)') title_tr = 'Elektronik Vuruş Alarmı (Bite Alarm)';
...
• İdeal Kullanım: Tatlı su göl ve barajlarında boili ve mısır ile sazan avı.`;
...
• Bait: 12-15 mm high-attract pop-up boilie.`;
    ...
    if (error) {
      console.error(`Failed to update ID ${art.id}:`, error.message);
    } else {
      successCount++;
    }
  }

  console.log(`Successfully updated ${successCount} / ${articles.length} articles directly in Supabase!`);
}
